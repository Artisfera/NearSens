...
uint16_t chooseClusterResult(uint16_t *values, uint8_t count, uint16_t clusterMm, uint8_t minCluster, bool *stable) {
  ...
  *stable = false;
  ...
  /*
    v33: do not blindly take the first stable cluster. Around the lower range a
    fixed ringdown/near-field lobe can sit near 100 mm and can make 80, 110,
    130 or 140 mm all look like "100" after calibration. At the same time we do
    not want the old pure "largest cluster wins" trap. The rule below chooses
    the best compact cluster: most members first, then smaller spread, then
    earlier distance only as the final tie-breaker.
  */
  uint8_t bestStart = 0;
  uint8_t bestMembers = 0;
  uint16_t bestSpread = 0xFFFF;
  uint16_t bestMedian = 0xFFFF;
  ...
  for (uint8_t start = 0; start < count; start++) {
    ...
    if (members > bestMembers) {
      better = true;
    } else if (members == bestMembers && spread < bestSpread) {
      better = true;
    } else if (members == bestMembers && spread == bestSpread && median < bestMedian) {
      better = true;
    }
