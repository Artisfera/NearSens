  ...
  if (displayMode == DISPLAY_BANNER) {
    if (dueMs(millis(), bannerUntilMs)) {
      if (lastMeasurementValid) {
        showMeasurementResult(lastRawMeasurementMm);
      } else {
        showIdleMarker();
      }
    }
...
void showReferenceBanner() {
  ...
  // User-requested offset state display:
  //   0 _ 75 = 75 mm offset OFF
  //   1 _ 75 = 75 mm offset ON
  // The second digit is intentionally blank.
...
void showRawModeBanner() {
  ...
  bannerUntilMs = millis() + REFERENCE_BANNER_MS;
  if (rawMode) {
    lcdShowValue(LCD_RAW_ON);
  } else {
    lcdShowValue(LCD_RAW_OFF);
  }
  physicallyDisplayedValue = 0xFFFF;
}
