...
void showMeasurementResult(uint16_t rawMm) {
  if (rawMode) {
    // RAW skips calibration, but still follows the visible 0 _ 75 / 1 _ 75
    // display reference selection so TS4 always changes what the LCD shows.
...
void toggleReferenceMode() {
  ...
  // UI-only banner. The actual display offset is applied once in showMeasurementResult().
  showReferenceBanner();
}
