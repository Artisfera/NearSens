...
void showMeasurementResult(uint16_t rawMm) {
  if (rawMode) {
    // RAW is diagnostic before calibration, but it still follows the selected
    // LCD reference mode. If the LCD banner shows 1 _ 75, every displayed
    // measurement, including RAW, must have 75 mm subtracted.
...
void toggleReferenceMode() {
  ...
  // UI-only banner. The actual 75 mm offset is applied exactly once in
  // showMeasurementResult(), to both calibrated and RAW displayed values.
  showReferenceBanner();
}
