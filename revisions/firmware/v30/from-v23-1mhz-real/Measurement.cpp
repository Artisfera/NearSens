...
static bool echoCandidateLooksValid(const EchoCandidate &candidate,
  ...
  bool strongPeak = candidate.peakDeviation >=
                    (uint16_t)(threshold + ADC_STRONG_CONFIRM_MARGIN);

  /*
    1 MHz note: at low CPU clock the ADC loop samples the echo envelope less
    densely. A real 40-50 cm echo can occasionally produce only one useful
    over-threshold sample. Near echoes still need two hits unless they are
    strong. Far echoes may pass with one hit, but only after range filtering;
    packet-level clustering still rejects random single-sample noise.
  */
  uint8_t requiredHits = farEcho ? 1 : ECHO_MIN_PULSE_HITS;
  if (candidate.hits < requiredHits && !strongPeak) {
    return false;
  }
  ...
  if (!farEcho && widthUs < ECHO_MIN_PULSE_WIDTH_US &&
      candidate.hits <= ECHO_MIN_PULSE_HITS && !strongPeak) {
    return false;
  }
...
uint16_t measureOnePing(bool *valid) {
  *valid = false;
  ...
  uint32_t farEchoStartUs = txStartUs +
    (((uint32_t)FAR_ECHO_START_MM * 2000UL + 342UL) / 343UL);
  ...
  while (true) {
    ...
    uint16_t activeThreshold = dueUs(sampleUs, farEchoStartUs) ? farThreshold : threshold;
    ...
    if (inPulse) {
      storeEchoCandidate(candidates, &candidateCount, txStartUs,
                         ...
                         dueUs(firstUs, farEchoStartUs) ? farThreshold : threshold);
      inPulse = false;
      firstUs = 0;
      peakUs = 0;
      peakDeviation = 0;
