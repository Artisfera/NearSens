...
static bool echoCandidateLooksValid(const EchoCandidate &candidate,
  ...
  uint16_t distanceMm = timeOfFlightToMm(candidate.firstUs - txStartUs);
...
static bool chooseBestEchoCandidate(EchoCandidate *candidates,
  ...
  for (uint8_t i = 1; i < candidateCount; i++) {
    ...
    if (candidates[i].peakDeviation >
      ...
      if (candidates[i].firstUs < candidates[bestIndex].firstUs) {
        better = true;
      }
    }
  ...
  *distanceMm = timeOfFlightToMm(candidates[bestIndex].firstUs - txStartUs);
  return true;
}
...
uint16_t captureBaseline(uint16_t *minAdc, uint16_t *maxAdc) {
  // Do not toggle LCD BP/PH inside the baseline sampling loop.
  // One clean BP service before the loop is enough, and it avoids injecting
  // digital switching noise into the ADC noise estimate.
  serviceBackplane();
