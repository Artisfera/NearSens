...
void showMeasurementResult(uint16_t rawMm) {
  if (rawMode) {
    // RAW is diagnostic: show the measurement before calibration and before the
    // 75 mm display offset. This must not change when TS4 toggles reference mode.
    setDisplayValue(rawMm);
    return;
  }
...
void toggleReferenceMode() {
  ...
  // UI-only banner. The actual offset is applied once in showMeasurementResult().
  showReferenceBanner();
}
