...
uint16_t measurePacket(uint8_t runType, bool *ok, uint16_t expectedMm) {
  ...
  if (validCount >= minValid) {
    ...
    if (runType == RUN_CALIBRATE && expectedMm > 0) {
      // During calibration the target distance is known. Pick the stable cluster
      // closest to that target, not blindly the nearest echo. This prevents a
      // repeatable early lobe from ruining 300/500 mm calibration.
      result = chooseClusterResultNear(values, validCount, clusterMm, minCluster, expectedMm, &stable);
    } else {
      result = chooseClusterResult(values, validCount, clusterMm, minCluster, &stable);
    }
...
static bool echoCandidateLooksValid(const EchoCandidate &candidate,
  ...
  return distanceMm >= ECHO_MIN_MM && distanceMm <= ECHO_MAX_MM;
}
...
static void storeEchoCandidate(EchoCandidate *candidates,
  ...
  if (!echoCandidateLooksValid(candidate, txStartUs, threshold)) {
    return;
  }
  ...
  while (true) {
    ...
    if (inPulse) {
      storeEchoCandidate(candidates, &candidateCount, txStartUs,
                         firstUs, peakUs, peakDeviation, hits, threshold);
      inPulse = false;
      firstUs = 0;
      peakUs = 0;
      peakDeviation = 0;
...
uint16_t chooseClusterResultNear(uint16_t *values, uint8_t count, uint16_t clusterMm, uint8_t minCluster, uint16_t expectedMm, bool *stable) {
  ...
  *stable = false;
  ...
  bool found = false;
  uint16_t bestValue = values[count / 2];
  ...
  for (uint8_t start = 0; start < count; start++) {
    ...
    if (members >= minCluster) {
      ...
      if ((members & 0x01) != 0) {
        clusterValue = values[middle];
      } else {
        clusterValue = (uint16_t)(((uint32_t)values[middle - 1] + values[middle] + 1UL) / 2UL);
      }

      uint16_t diff = absDiff16(clusterValue, expectedMm);
