...
bool captureCalibrationPoint(uint16_t targetMm, uint16_t *rawMm, uint16_t *errorCode) {
  ...
  for (uint8_t i = 0; i < CAL_POINT_SAMPLES; i++) {
    ...
    bool ok = false;
    uint16_t sample = measurePacket(RUN_CALIBRATE, &ok, targetMm);
  ...
  uint16_t medianRaw = samples[goodCount / 2];
  if (absDiff16(medianRaw, targetMm) > CAL_TARGET_MAX_DEVIATION_MM) {
    *errorCode = LCD_CAL_FAIL;
    return false;
  }
