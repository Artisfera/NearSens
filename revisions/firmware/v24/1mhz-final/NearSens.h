...
// NearSens v24 1MHz final profile.
// Tuning goal: reliable echo capture and calibration at F_CPU = 1 MHz.
...
static const uint16_t CAL_POINT_MAX_SPREAD_MM = 45;
static const uint16_t CAL_TARGET_MAX_DEVIATION_MM = 170;
...
uint16_t measurePacket(uint8_t runType, bool *ok, uint16_t expectedMm = 0);
...
uint16_t chooseClusterResultNear(uint16_t *values, uint8_t count, uint16_t clusterMm, uint8_t minCluster, uint16_t expectedMm, bool *stable);
