...
uint16_t chooseClusterResult(uint16_t *values, uint8_t count, uint16_t clusterMm, uint8_t minCluster, bool *stable) {
  ...
  *stable = false;
  ...
  uint16_t bestSpan = 65535U;

  for (uint8_t start = 0; start < count; start++) {
    for (uint8_t end = start; end < count; end++) {
      uint16_t span = values[end] - values[start];
      if (span > clusterMm) {
        break;
      }
      ...
      if (members > bestMembers ||
          (members == bestMembers && span < bestSpan)) {
        bestMembers = members;
        bestSpan = span;
        bestStart = start;
      }
  ...
  *stable = true;
  ...
  if (bestMembers >= 5) {
    uint32_t sum = (uint32_t)values[middle - 1] +
                   values[middle] +
                   values[middle + 1];
    return (uint16_t)((sum + 1UL) / 3UL);
  }
