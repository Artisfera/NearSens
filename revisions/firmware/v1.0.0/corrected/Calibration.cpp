...
bool applyCalibration(uint16_t rawMm, uint16_t *correctedMm) {
  ...
  // The function returns the calibrated result, still measured from the sensor side.
  // The 75 mm correction is separate and only applies to the value shown on the LCD.
...
void startCalibrationWizard() {
  ...
  calibrationWizardStep = 2;  // Start od point 300 mm, zwykle najwygodniejszego do setienia.
  ts3ClickCount = 0;
