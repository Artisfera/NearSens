...
static uint16_t applyDisplayReferenceOffset(uint16_t frontMm) {
  ...
  // BODY mode shows the result referenced to the rear side of the enclosure.
  // The 75 mm correction only affects the value shown on the LCD. It does not change RAW or
  // the calibration points stored in EEPROM.
...
void showMeasurementResult(uint16_t rawMm) {
  if (rawMode) {
    // RAW is a diagnostic mode. It shows the measurement before calibration
    // and before the 75 mm correction. Toggling TS4 does not change the RAW result.
    setDisplayValue(rawMm);
    return;
  }
...
void toggleReferenceMode() {
  ...
  // This is only an LCD message. The correction itself is applied only
  // when displaying the normal measurement result.
  showReferenceBanner();
}
