...
static bool chooseBestEchoCandidate(EchoCandidate *candidates,
  ...
  for (uint8_t i = 1; i < candidateCount; i++) {
    ...
    if (candidates[i].peakDeviation >
      ...
      // With similar amplitude, the earlier echo is selected.
      // This prototype works over a short range, so this is the safer choice.
...
uint16_t measureOnePing(bool *valid) {
  *valid = false;
  ...
  // Digital output switching is limited during the receive window.
  // Echo measurement is more important than a single LCD refresh delay.
...
uint16_t chooseClusterResult(uint16_t *values, uint8_t count, uint16_t clusterMm, uint8_t minCluster, bool *stable) {
  ...
  *stable = false;
  ...
  /*
    For this signal path, taking the first stable cluster is better than taking the largest one.
    A later reflection can be repeatable, but it does not have to be a reflection from
    the tested object. In the 10-50 cm range, the nearest
    stable result is safer.
  */
