...
uint16_t measureOnePing(bool *valid) {
  *valid = false;
  ...
  // Secondary threshold for weaker, farther echoes. It is used only after
  // FAR_ECHO_START_MM, so it does not make the close-range ringdown easier
  // to accept. This improves 1 MHz operation where the ADC scan is sparse
  // and far echoes are weaker.
  ...
  while (true) {
    ...
    uint16_t sampleDistanceMm = timeOfFlightToMm(sampleUs - txStartUs);
    uint16_t activeThreshold = (sampleDistanceMm >= FAR_ECHO_START_MM) ? farThreshold : threshold;
    ...
    if (inPulse) {
      uint16_t candidateDistanceMm = timeOfFlightToMm(peakUs - txStartUs);
      uint16_t candidateThreshold = (candidateDistanceMm >= FAR_ECHO_START_MM) ? farThreshold : threshold;
      storeEchoCandidate(candidates, &candidateCount, txStartUs,
                         firstUs, peakUs, peakDeviation, hits, candidateThreshold);
      inPulse = false;
      firstUs = 0;
      peakUs = 0;
      peakDeviation = 0;
...
uint16_t captureBaseline(uint16_t *minAdc, uint16_t *maxAdc) {
  // Keep ADC baseline quiet. Servicing BP/PH inside this loop can change the
  // measured noise and move the dynamic threshold by a few ADC counts.
  serviceBackplane();
  uint32_t sum = 0;
  *minAdc = 1023;
  *maxAdc = 0;
