...
bool applyCalibration(uint16_t rawMm, uint16_t *correctedMm) {
  ...
  */
  ...
  /*
    Reference modes:
    - REFERENCE_FRONT: display the distance measured from the ultrasonic
      transducer/front face. No housing offset is applied.
    - REFERENCE_BODY: display the equivalent distance from the selected
      body/reference edge. In this build the body reference is shifted by
      DEVICE_REFERENCE_OFFSET_MM, so the displayed distance must be SMALLER
      than the front-face distance. This offset is applied exactly once and
      only in this normal calibrated display path. RAW mode and calibration
